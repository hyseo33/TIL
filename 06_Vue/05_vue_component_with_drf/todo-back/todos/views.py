...
def todo_create(request):
    ...
    if serializer.is_valid():
        serializer.save()
        # serializer.data -> Json ({'id': 1, 'user': 1, 'title': 첫번째 입력, 'completed': false})
...
def todo_update_delete(request, id):
    ...
    if request.method == 'PUT':
        ...
        todo.delete()
        # 204 -> 해당하는 컨텐츠가 없는 경우(todo를 삭제 했기 때문에 해당 todo가 존재하지 않음을 알려줌)
...
def user_singup(request):
    ...
    if serializer.is_valid(raise_exception=True):
        # serializer.save()의 리턴 값은 User 모델의 인스턴스
